...
class Plotter:
    def __init__(self):
        ...
        self.from_m1_page_topleft, self.from_m2_page_topleft = Vector(-302, 84), Vector(65, 84)
        self.string_lengths = self.loc_to_string_lengths(Vector(0,0)) # top left of page
        debug(DEBUG, self.string_lengths.format())
        # Resolution, scale and speed
        self.resolution = 1 # (in mm) resolution of segments within the line. TODO: Change to less later
        self.drawing_speed = self.S # M, I, S, D
    ...
    def line_to(self, new_x, new_y, pen_down=True):
        debug(DEBUG, 'line_to ({},{} to {},{})'.format(self.loc.x, self.loc.y, new_x, new_y))        
        # limit x,y values to page size
        new_x, new_y = limit(new_x, 0, self.page_w), limit(new_y, 0, self.page_h)
        new_loc = Vector(new_x, new_y).mult(self.plotter_global_scale)
        # pen down/up
        self.pen.down(pen_down)
        # create midpoints (break each long line to short lines of 'resolution' length)
        midpoints = get_midpoints(self.loc, new_loc, self.resolution)
        ...
        for new_loc in midpoints:
            ...
            actual_change_in_strings = Vector(\
                actual_motor_steps.x*self.one_step_in_mm*(-1), actual_motor_steps.y*self.one_step_in_mm)
            self.string_lengths.add(actual_change_in_strings)
            self.loc.copy(new_loc) # NO! -- the new location should figure out where it is based on the actual change in the strings
    ...
    def draw_pic(self, coords):
        for line in coords:
            for c in range(0, len(line), 2):
                self.line_to(line[c], line[c+1], False if c==0 else True)
    # returns string vector (string_length_m1, string_length_m2) at location v 
    def loc_to_string_lengths(self, loc):
        m1_string_length = loc.dup().add(self.from_m1_page_topleft).mag()
        m2_string_length = loc.dup().add(self.from_m2_page_topleft).mag()
    ...
    def get_motor_steps(self, change_in_strings):
        m1_steps = math.floor(change_in_strings.x/self.one_step_in_mm) * (-1) # fwd is counter-clockwise
        m2_steps = math.floor(change_in_strings.y/self.one_step_in_mm) 
    ...
    def spin_motors_parallel(self, steps):
        ...
        m1_dir = self.back if m1_steps < 0 else self.fwd
        m2_dir = self.back if m2_steps < 0 else self.fwd
        # steps in single spin of style
        steps_per_spin, style = self.drawing_speed["steps"], self.drawing_speed["style"]
        m1_spins = m1_spins_init = abs(m1_steps//steps_per_spin)
        m2_spins = m2_spins_init = abs(m2_steps//steps_per_spin)
        ...
        for i in range (len(midpoints)-1, -1, -1): #.. until -1 because range excludes upper (lower in this case) bound
            if m1_spins > midpoints[i].x: 
                self.motor_spin(self.m1, m1_dir, 1, style)
                m1_spins -= 1
            if m2_spins > midpoints[i].y: 
                self.motor_spin(self.m2, m2_dir, 1, style)
                m2_spins -= 1 
